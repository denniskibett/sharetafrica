...
<div x-data="tenancyDeleteModal" x-init="init()">
...
document.addEventListener('alpine:init', () => {
  Alpine.data('tenancyDeleteModal', () => ({
    isOpen: false,
    tenancyToDelete: null,
    message: '',
    
    init() {
      window.tenancyDeleteModal = this;
    },
    
    openModal(tenancy) {
      this.tenancyToDelete = tenancy;
      this.message = `Are you sure you want to delete the tenancy for "${tenancy.tenant_name || 'Unknown'}" in unit "${tenancy.unit_number || 'Unknown'}"? This action cannot be undone and will set the unit status to vacant.`;
      this.isOpen = true;
    ...
    closeModal() {
      this.isOpen = false;
      this.tenancyToDelete = null;
      document.body.style.overflow = '';
    },
    
    confirmDelete() {
      if (this.tenancyToDelete) {
        const form = document.createElement('form');
        form.method = 'POST';
        form.action = `/tenancies/${this.tenancyToDelete.id}`;
