...
<div x-data="unitDeleteModal" x-init="init()">
...
document.addEventListener('alpine:init', () => {
  Alpine.data('unitDeleteModal', () => ({
    isOpen: false,
    unitToDelete: null,
    message: '',
    
    init() {
      window.unitDeleteModal = this;
    },
    
    openModal(unit) {
      this.unitToDelete = unit;
      this.message = `Are you sure you want to delete unit "${unit.unit_number}"? This action cannot be undone.`;
      this.isOpen = true;
    ...
    confirmDelete() {
      if (this.unitToDelete) {
        ...
        form.method = 'POST';
        form.action = `/units/${this.unitToDelete.id}`;
