...
<div x-data="tenantDeleteModal" x-init="init()">
...
document.addEventListener('alpine:init', () => {
  Alpine.data('tenantDeleteModal', () => ({
    isOpen: false,
    tenantToDelete: null,
    message: '',
    
    init() {
      window.tenantDeleteModal = this;
    },
    
    openModal(tenant) {
      this.tenantToDelete = tenant;
      this.message = `Are you sure you want to delete tenant "${tenant.name}"? This will also delete their user account and end any active tenancies. This action cannot be undone.`;
      this.isOpen = true;
    ...
    confirmDelete() {
      if (this.tenantToDelete) {
        ...
        form.method = 'POST';
        form.action = `/tenants/${this.tenantToDelete.id}`;
