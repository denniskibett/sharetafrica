  ...
  <div x-show="isOpen" 
    ...
    <div class="p-6 lg:p-10">
      ...
      <form @submit.prevent="submitForm()">
        @csrf
        <h4 class="mb-6 text-lg font-medium text-gray-800 dark:text-white/90">Add New Expense Category</h4>
        ...
        <div class="grid grid-cols-1 gap-y-5">
          <div>
            ...
            <input x-model="formData.name" type="text" name="name" required class="dark:bg-dark-900 h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 shadow-theme-xs placeholder:text-gray-400 focus:border-blue-300 focus:outline-hidden focus:ring-3 focus:ring-blue-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30" placeholder="Enter category name" />
          </div>

          <div>
            <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">Description</label>
            <textarea x-model="formData.description" name="description" rows="3" class="dark:bg-dark-900 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 shadow-theme-xs placeholder:text-gray-400 focus:border-blue-300 focus:outline-hidden focus:ring-3 focus:ring-blue-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30" placeholder="Enter category description (optional)"></textarea>
          </div>
        </div>
  ...
  Alpine.data('categoryCreateModal', () => ({
    ...
    async submitForm() {
      ...
      try {
        ...
        const response = await fetch('/expense-categories', {
          method: 'POST',
          headers: {
        ...
        if (response.ok && data.success) {
          ...
          else this.formErrors = [data.message || 'Failed to create category. Please try again.'];
          const modalContent = document.querySelector('.overflow-y-auto');
          if (modalContent) modalContent.scrollTop = 0;
        }
      } catch (error) {
